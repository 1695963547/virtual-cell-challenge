...
class Data:
    ...
    def load_data(self, data_name = None, data_path = None):
        ...
        if data_name in ['norman', 'norman_umi_go_filtered',]:
            ...
            self.adata = sc.read_h5ad(os.path.join(self.data_path, data_name + '.h5ad'))
        elif data_name in ['vcc', ]:
            # VCC: 训练用 vcc_train.h5ad（6 文件合并），验证用官方 val 转的 vcc_val.h5ad
            self.adata = sc.read_h5ad(os.path.join(self.data_path, 'vcc_train.h5ad'))
            self.adata_val = sc.read_h5ad(os.path.join(self.data_path, 'vcc_val.h5ad'))
        else:
    ...
    def process_data(self, n_top_genes = 2000,infer_top_gene=1000,split_method='additive',
        ...
        if self.data_name == 'combosciplex':
            ...
            # VCC: X 已是 log1p(CP10K) 归一化（build_scdfm_data.py 已做 normalize_total(1e4)+log1p）、
            # 全基因（18080），跳过归一化/HVG
            for adata_ in (self.adata, self.adata_val):
                adata_.var['highly_variable'] = True
                adata_.obs['is_control'] = (adata_.obs['condition'] == 'control').to_numpy()

            self.adata_train = self.adata
            self.adata_test = self.adata_val

            # perturbation_dict 取 train∪val 的 condition（val 评测时也要能反查）
            condition = np.unique(np.concatenate([
                self.adata.obs['condition'], self.adata_val.obs['condition']]))
            unique_perturbation = np.unique(condition)
            unique_perturbation.sort()
            self.unique_perturbation = unique_perturbation
    ...
    def load_flow_data(self, batch_size = 128):
        if self.data_name == 'combosciplex':
            ...
            train_sampler = TrainSampler(self.data_name, self.adata_train, ["condition"], self.perturbation_dict)
            test_sampler = TestDataset(self.data_name, self.adata_test, ["condition"], self.perturbation_dict)
            return train_sampler , test_sampler, []
        else:
            raise ValueError(self.data_name + ' is not a valid data name')
...
class TrainSampler:
    def __init__(self, data_name, adata: sc.AnnData, perturbation_covariates: list[str], perturbation_dict: dict,):
        ...
        ctrl_name = 'control+control' if data_name != 'vcc' else 'control'
        self._perturbation_covariates = self._perturbation_covariates[self._perturbation_covariates != ctrl_name]
...
class TestDataset:
    ...
    def get_control_data(self,):
        ...
        if 'cell_line_id' in self.adata.obs:
            res['cell_line_id'] = control_data.obs['cell_line_id'].to_numpy()
        return res
    ...
    def get_perturbation_data(self, perturbation: str):
        ...
        if 'cell_line_id' in self.adata.obs:
            cl = perturbation_data.obs['cell_line_id'].unique()
            res['cell_line_id'] = int(cl[0]) if len(cl) == 1 else cl.tolist()
        return res
...
class PerturbationDataset(Dataset):
    def __init__(self, sampler: TrainSampler, batch_size: int):
        ...
        ctrl_name = 'control+control' if sampler.data_name != 'vcc' else 'control'
        self.control_idx = (sampler.adata.obs['perturbation_covariates'] == ctrl_name).to_numpy().nonzero()[0]
        
        # VCC: 按 cell line 分层，source control 与 target 同系
        self.use_cell_line = 'cell_line_id' in sampler.adata.obs
        if self.use_cell_line:
            self.cell_line_arr = sampler.adata.obs['cell_line_id'].to_numpy()
            self.control_by_cell_line = {
                cl: self.control_idx[self.cell_line_arr[self.control_idx] == cl]
                for cl in np.unique(self.cell_line_arr)
            }
    ...
    def __getitem__(self, idx):
        ...
        if self.use_cell_line:
            # 若该 condition 跨 cell line，收敛到样本数最多的系，保证 target 单系
            cl_counts = np.bincount(self.cell_line_arr[tgt_idx], minlength=4)
            cl = int(np.argmax(cl_counts))
            tgt_idx = tgt_idx[self.cell_line_arr[tgt_idx] == cl]
        tgt_batch_idx = np.random.choice(tgt_idx, self.batch_size)
        
        # source (control) batch：vcc 取同 cell line 的 control
        if self.use_cell_line:
            src_batch_idx = np.random.choice(self.control_by_cell_line[cl], self.batch_size)
        else:
        ...
        if self.use_cell_line:
            out['cell_line_id'] = torch.tensor(self.cell_line_arr[tgt_batch_idx])
        return out
