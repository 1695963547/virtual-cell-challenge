#!/usr/bin/env bash
# Route B-3: state_lg + Fused Loss + AdamW (cs512)
# 验证：大模型(301M) + 基因空间 fused loss + AdamW 优化器
# 与 fused_run2 的区别：state_lg(301M) vs state(151M), cs=512 vs 128, AdamW vs Adam
#
# 关键参数：
#   MODEL=state_lg, embed_key=null, output_space=gene
#   loss=fused, optimizer=adamw, cell_set_len=512
#
# 用法：
#   bash /home/zjh/state-vcc-local/run_train_state_lg_fused_adamw.sh
# 日志：/home/zjh/log/train_state_lg_fused_adamw.log

set -uo pipefail

ROOT=/home/zjh/state-vcc-local
STATE=$ROOT/state
NAME=${NAME:-state_lg_fused_adamw}
...
log "run=$NAME  model=$MODEL  max_steps=$MAX_STEPS  loss=fused  optimizer=adamw  cell_set_len=$CELL_SET_LEN"

log "===== 启动训练（state_lg + Fused Loss + AdamW） ====="
"$STATE/.venv/bin/state" tx train \
  ...
  +model.kwargs.optimizer=adamw \
  +model.kwargs.weight_decay=0.01 \
