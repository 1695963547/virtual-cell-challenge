...
class PseudobulkPerturbationModel(PerturbationModel):
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int, padded=True) -> torch.Tensor:
        ...
        if isinstance(self.loss_fn, FusedPDSDESLoss):
            # Fused PDS/DES proxy loss: compute delta at pseudo-bulk level
            # pred and target are [B, S, output_dim], all cells in a sample share the same pred
            delta_pred = pred.mean(dim=1)       # [B, G]
            delta_true = target.mean(dim=1)     # [B, G]

            # Subtract control baseline to get actual delta
            ctrl_key = "ctrl_cell_counts" if "ctrl_cell_counts" in batch else "ctrl_cell_emb"
            ctrl = batch[ctrl_key]
            if padded:
                ctrl = ctrl.reshape(-1, self.cell_sentence_len, ctrl.shape[-1])
            else:
                ctrl = ctrl.reshape(1, -1, ctrl.shape[-1])
            delta_pred = delta_pred - ctrl.mean(dim=1)  # pred - control = delta
            delta_true = delta_true - ctrl.mean(dim=1)  # true - control = delta
    ...
    def validation_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> None:
        ...
        if isinstance(self.loss_fn, FusedPDSDESLoss):
            # Fused loss: compute delta at pseudo-bulk level
            delta_pred = pred.mean(dim=1)
            delta_true = target.mean(dim=1)
            ctrl_key = "ctrl_cell_counts" if "ctrl_cell_counts" in batch else "ctrl_cell_emb"
            ctrl_raw = batch[ctrl_key]
            if ctrl_raw.dim() == 2:
                ctrl = ctrl_raw.reshape(-1, self.cell_sentence_len, ctrl_raw.shape[-1])
            else:
                ctrl = ctrl_raw
            delta_pred = delta_pred - ctrl.mean(dim=1)
            delta_true = delta_true - ctrl.mean(dim=1)
            loss, _ = self.loss_fn(delta_pred, delta_true)
        else:
