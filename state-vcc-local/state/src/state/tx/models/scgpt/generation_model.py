...
class DrugEncoder(nn.Module):
    def __init__(self, d_model: int, n_drug_tokens: int):
        super().__init__()
        self.embedding = nn.Embedding(n_drug_tokens, d_model)
...
class TransformerGenerator(nn.Module):
    ...
    def pred_perturb(
        ...
        x: torch.Tensor = batch_data.x.reshape(batch_size, -1)
...
class ChemicalTransformerGenerator(nn.Module):
    def __init__(
        ...
        self.encoder = GeneEncoder(ntoken, d_model, padding_idx=pad_token_id)
        self.drug_encoder = DrugEncoder(d_model, n_drug_tokens)
        self.value_encoder = ContinuousValueEncoder(d_model, dropout)
        self.pert_encoder = nn.Embedding(3, d_model, padding_idx=pert_pad_id)
