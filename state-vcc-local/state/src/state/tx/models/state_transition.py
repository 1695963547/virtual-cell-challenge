...
from .fused_loss import FusedPDSDESLoss
...
class StateTransitionPerturbationModel(PerturbationModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder_loss_type = kwargs.get("decoder_loss_type", "energy")  # "energy" | "delta_mse"
        ...
        elif loss_name == "fused":
            self.loss_fn = FusedPDSDESLoss(
                w_mse=kwargs.get("fused_w_mse", 1.0),
                w_pds=kwargs.get("fused_w_pds", 0.1),
                w_dir=kwargs.get("fused_w_dir", 0.1),
                alpha=kwargs.get("fused_alpha", 5.0),
                tau=kwargs.get("fused_tau", 10.0),
            )
        else:
        ...
        # Route B: freeze_main=True → freeze everything except gene_decoder
        # Used when init_from provides a converged main network and only decoder needs retraining
        self.freeze_main = kwargs.get("freeze_main", False)
        if self.freeze_main:
            for module_name in ["pert_encoder", "basal_encoder", "transformer_backbone",
                                "project_out", "final_down_then_up"]:
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int, padded=True) -> torch.Tensor:
        ...
        # Fused loss: compute pseudo-bulk delta in gene space
        if isinstance(self.loss_fn, FusedPDSDESLoss):
            ctrl = batch["ctrl_cell_emb"]
            if padded:
                ctrl = ctrl.reshape(-1, self.cell_sentence_len, self.output_dim)
            else:
                ctrl = ctrl.reshape(1, -1, self.output_dim)
            delta_pred = pred.mean(dim=1) - ctrl.mean(dim=1)   # [B, G]
            delta_true = target.mean(dim=1) - ctrl.mean(dim=1)  # [B, G]
            main_loss, components = self.loss_fn(delta_pred, delta_true)
            self.log("train_loss", main_loss)
            self.log("fused_wmse", components["weighted_mse"])
            self.log("fused_pds", components["pds_loss"])
            self.log("fused_dir", components["direction_loss"])
            per_set_main_losses = main_loss.unsqueeze(0)  # for confidence target
        else:
        ...
        # Decoder loss: gene-space delta MSE or legacy energy distance
        if self.gene_decoder is not None and "pert_cell_counts" in batch:
            ...
            if self.decoder_loss_type == "delta_mse":
                ctrl_latent = batch["ctrl_cell_emb"].reshape_as(pred).detach()
                gene_ctrl = self.gene_decoder(ctrl_latent)
                delta_pred = pert_cell_counts_preds - gene_ctrl
                delta_true = gene_targets - gene_ctrl
                decoder_loss = F.mse_loss(delta_pred, delta_true)
                self.log("decoder_delta_mse", decoder_loss)
            else:
    ...
    def validation_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> None:
        ...
        # Fused loss: compute pseudo-bulk delta in gene space
        if isinstance(self.loss_fn, FusedPDSDESLoss):
            ctrl = batch["ctrl_cell_emb"].reshape(-1, self.cell_sentence_len, self.output_dim)
            delta_pred = pred.mean(dim=1) - ctrl.mean(dim=1)   # [B, G]
            delta_true = target.mean(dim=1) - ctrl.mean(dim=1)  # [B, G]
            val_loss, components = self.loss_fn(delta_pred, delta_true)
            self.log("val_loss", val_loss)
            self.log("val/fused_wmse", components["weighted_mse"])
            self.log("val/fused_pds", components["pds_loss"])
            self.log("val/fused_dir", components["direction_loss"])
            per_set_main_losses = val_loss.unsqueeze(0)  # for confidence target
            loss = val_loss
        else:
            per_set_main_losses = self._compute_distribution_loss(pred, target)
        ...
        if self.gene_decoder is not None and "pert_cell_counts" in batch:
            ...
            if self.decoder_loss_type == "delta_mse":
                ...
                delta_true = gene_targets - gene_ctrl
                decoder_loss = F.mse_loss(delta_pred, delta_true)
                self.log("val/decoder_delta_mse", decoder_loss)
            else:
                decoder_per_set = self._compute_distribution_loss(pert_cell_counts_preds, gene_targets)
                decoder_loss = decoder_per_set.mean()
