...
def get_checkpoint_callbacks(
    ...
    Returns a list of callbacks. The best checkpoint callback is always first,
    since the caller reads callbacks[0].dirpath to locate last.ckpt for resuming.
    """
    checkpoint_dir = join(output_dir, name, "checkpoints")
    callbacks = []

    # Best checkpoint (by val_loss). It must NOT set every_n_train_steps: with a
    # monitored validation metric, saving at train-batch end evaluates val_loss one
    # validation interval behind the current weights, so the saved weights would not
    # match the recorded best score (best.ckpt would not be the true best). Leaving
    # the trigger unset makes Lightning save at the end of each validation, where
    # val_loss was just computed on these exact weights, keeping them aligned.
    ...
    # Latest checkpoint for resuming: written every val_freq training steps regardless
    # of the monitored metric, so a resume always restarts from the most recent step.
    ...
    # Opt-in: also keep every intermediate checkpoint, so that metrics can be
    # evaluated as a function of training step after the run finishes.
    if keep_all_ckpts and ckpt_every_n_steps and ckpt_every_n_steps > 0:
