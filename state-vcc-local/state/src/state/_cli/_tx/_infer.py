        ...
        for g in unique_groups:
            ...
            for p in pbar:
                ...
                while start < len(idxs):
                    ...
                    # Fix: when counts_expected (output_space in {gene,all} with embed_key),
                    # prefer pert_cell_counts_preds (gene-space) regardless of writes_to.
                    # Previously this branch was gated on writes_to[0]==".X", causing
                    # output_space=all + --embed-key to silently fall through to the
                    # embedding-space preds and never decode to gene space.
                    if (
                        counts_expected
