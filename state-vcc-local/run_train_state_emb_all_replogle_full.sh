#\!/usr/bin/env bash
# +replogle_full 合训：与 state_emb_all_lg_cs512_valsplit 完全同配方，
# 唯一变量 = toml 换成 starter_valsplit_replogle.toml（新增 Replogle K562 GWPS 全量
# 1,961,299 细胞 train 数据）。消融矩阵第三格：
#   state_lg 老（val 锚点 0.0578）→ valsplit（0.0478）→ +replogle_full（本实验）
#
# 复刻 state_lg 配方（与 run_train_state_emb_all_valsplit.sh 逐参数一致）：
...
#
# 用法（Qoder 后台或用户终端）：
#   bash /home/zjh/state-vcc-local/run_train_state_emb_all_replogle_full.sh \
#     > /home/zjh/log/train_state_emb_all_replogle_full.log 2>&1
...
STATE=$ROOT/state
NAME=${NAME:-state_emb_all_lg_cs512_replogle_full}
...
log "===== 前置检查 ====="
for f in vci_pretrain/starter_valsplit_replogle.toml \
         vci_pretrain/replogle_full_trainready.h5ad \
...
log "===== 启动训练（valsplit 同配方 + Replogle 全量合训） ====="
"$STATE/.venv/bin/state" tx train \
  data.kwargs.toml_config_path=vci_pretrain/starter_valsplit_replogle.toml \
